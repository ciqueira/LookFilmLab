...
struct KernelParams {
  ...
  int32_t inputPrimariesColorSpace;
  int32_t inputTransferColorSpace;
  int32_t outputPrimariesColorSpace;
  int32_t outputTransferColorSpace;
  int32_t outputRole;
  int32_t hdrPreset;
  int32_t hdrTransfer;
  ...
  uint32_t _padPerf0;
  int32_t printSource;
  int32_t scanInputEncoding;
  int32_t scanInputColorSpace;
  int32_t scanWorkingColorSpace;
  float scanFilmBaseR;
  float scanFilmBaseG;
  float scanFilmBaseB;
...
KernelParams toKernelParams(const RenderParams &params, double time, int32_t width, int32_t height) {
  ...
  out.inputPrimariesColorSpace = static_cast<int32_t>(params.inputPrimariesColorSpace);
  out.inputTransferColorSpace = static_cast<int32_t>(params.inputTransferColorSpace);
  out.outputPrimariesColorSpace = static_cast<int32_t>(params.outputPrimariesColorSpace);
  out.outputTransferColorSpace = static_cast<int32_t>(params.outputTransferColorSpace);
  ...
  out.printSource = static_cast<int32_t>(params.printSource);
  out.scanInputEncoding = static_cast<int32_t>(params.scanInputEncoding);
  out.scanInputColorSpace = static_cast<int32_t>(params.scanInputColorSpace);
  out.scanWorkingColorSpace = static_cast<int32_t>(params.scanWorkingColorSpace);
  out.scanDensityBasis = static_cast<int32_t>(params.scanDensityBasis);
  out.scanFilmBaseR = params.scanFilmBaseR;
  out.scanFilmBaseG = params.scanFilmBaseG;
  out.scanFilmBaseB = params.scanFilmBaseB;
  out.scanBlackFlareR = params.scanBlackFlareR;
  out.scanBlackFlareG = params.scanBlackFlareG;
  out.scanBlackFlareB = params.scanBlackFlareB;
  out.scanExposureEv = params.scanExposureEv;
  out.scanDensityContrast = params.scanDensityContrast;
  out.scanDensityScaleR = params.scanDensityScaleR;
  out.scanDensityScaleG = params.scanDensityScaleG;
  out.scanDensityScaleB = params.scanDensityScaleB;
  out.scanDensityOffsetR = params.scanDensityOffsetR;
  out.scanDensityOffsetG = params.scanDensityOffsetG;
  out.scanDensityOffsetB = params.scanDensityOffsetB;
...
float autoExposureMeterY(float r, float g, float b, const RenderParams &params) {
  const int32_t transferColorSpace = std::clamp(
    static_cast<int32_t>(params.inputTransferColorSpace),
    0,
    static_cast<int32_t>(kSpektraColorSpaceCount - 1u)
  );
  const int32_t primariesColorSpace = std::clamp(
    static_cast<int32_t>(params.inputPrimariesColorSpace),
    0,
    static_cast<int32_t>(kSpektraColorSpaceCount - 1u)
  );
  const uint32_t *transferKinds = colorTransferKinds();
  if (transferKinds && transferKinds[transferColorSpace] != 0u) {
    const float *decodeLuts = colorDecodeLuts();
    r = sampleTransferLut(r, transferColorSpace, decodeLuts);
    g = sampleTransferLut(g, transferColorSpace, decodeLuts);
    b = sampleTransferLut(b, transferColorSpace, decodeLuts);
  }
  const float *meterMatrices = inputMeterXyzMatrices();
  if (!meterMatrices) {
    return 0.2126f * r + 0.7152f * g + 0.0722f * b;
  }
  const float *matrix = meterMatrices + static_cast<size_t>(primariesColorSpace) * 9u;
...
std::array<float, 3> scanFilmBaseTempTintFactors(const RenderParams &params) {
  const float temp = std::clamp(params.scanFilmBaseTemp, -100.0f, 100.0f) / 100.0f;
  const float tint = std::clamp(params.scanFilmBaseTint, -100.0f, 100.0f) / 100.0f;
  return {
    std::exp2(0.75f * temp),
    std::exp2(0.50f * tint),
    std::exp2(-0.75f * temp)
...
std::array<float, 3> scanFilmBaseColorRgb(const RenderParams &params) {
  return {
    std::clamp(params.scanFilmBaseColorR, 0.001f, 4.0f),
    std::clamp(params.scanFilmBaseColorG, 0.001f, 4.0f),
    std::clamp(params.scanFilmBaseColorB, 0.001f, 4.0f)
  };
}

void applyScanFilmBaseCalibration(
  RenderParams &params,
  const float *source,
  int32_t width,
  int32_t height
  ...
  (void)width;
  (void)height;
  std::array<float, 3> base = scanFilmBaseColorRgb(params);
  const std::array<float, 3> balance = scanFilmBaseTempTintFactors(params);
  params.scanFilmBaseR *= base[0] * balance[0];
  params.scanFilmBaseG *= base[1] * balance[1];
  params.scanFilmBaseB *= base[2] * balance[2];
}
...
struct MetalRenderer::Impl {
  ...
  double timestampTicksToMilliseconds(uint64_t ticks) const {
    ...
    if (@available(macOS 26.0, *)) {
      const SEL queryTimestampFrequencySelector = @selector(queryTimestampFrequency);
      id deviceObject = static_cast<id>(device);
      if ([deviceObject respondsToSelector:queryTimestampFrequencySelector]) {
        NSMethodSignature *signature = [deviceObject methodSignatureForSelector:queryTimestampFrequencySelector];
        uint64_t queried = 0ull;
        if (signature && [signature methodReturnLength] == sizeof(queried)) {
          NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:signature];
          [invocation setTarget:deviceObject];
          [invocation setSelector:queryTimestampFrequencySelector];
          [invocation invoke];
          [invocation getReturnValue:&queried];
        }
        if (queried > 0ull) {
          frequency = queried;
        }
...
bool MetalRenderer::render(
  ...
  @autoreleasepool {
    ...
    const bool needsFilmDensityPath =
      densityOutput ||
    ...
    const bool needsFrameConstants = needsFilmDensityPath || printDiffusionPath || finalProcessNegative || scannedNegativePrintBypass;
    ...
    id<MTLBuffer> diffusionDownsampleBlurBuffer = diffusionDownsamplePath
      ...
      : nil;
    id<MTLBuffer> printRawBufferA = (printDiffusionPath || printStageOutput || stagedFinalCorePath || finalProcessNegative || scannedNegativePrintBypass)
    ...
    if ((printDiffusionPath || printStageOutput || finalProcessNegative || scannedNegativePrintBypass) && !printRawBufferA) {
    ...
    RenderParams measuredParams = params;
    applyScanFilmBaseCalibration(
      measuredParams,
    ...
    KernelParams kernelParams = toKernelParams(measuredParams, time, width, height);
    ...
    applyProfileHalationDefaults(kernelParams, measuredParams, *filmCurves);
    ...
    auto encodePrintRawFromFilmDensity = [&](id<MTLBuffer> filmDensityBuffer, id<MTLBuffer> printRawBuffer) {
      ...
      [encoder setBuffer:colorDecodeLutBuffer offset:0 atIndex:22];
      [encoder setBuffer:colorTransferKindBuffer offset:0 atIndex:23];
    ...
    } else if (scannedNegativePrintBypass) {
      if (printDiffusionPath) {
        encodePrintDiffusionFromFilmDensity(renderSourceBuffer);
      } else {
        encodePrintRawFromFilmDensity(renderSourceBuffer, printRawBufferA);
        encodePrintStageOutputFromRaw(printRawBufferA);
      }
