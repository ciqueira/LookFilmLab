...
struct VulkanCorePushConstants {
  ...
  int32_t inputPrimariesColorSpace = 0;
  int32_t inputTransferColorSpace = 0;
};

static_assert(sizeof(VulkanCorePushConstants) == 112u);
...
constexpr uint32_t kCoreFrameFloatCount = 119u;
constexpr uint32_t kCoreFrameIntCount = 38u;
...
bool VulkanRenderer::Impl::renderCoreBootstrap(
  ...
  const bool dirPath = dirFeatureEnabled && !finalProcessNegative && !scannedNegativePrintBypass && params.dirCouplersAmount > 0.0f;
  ...
  if (!fullFrameSourcePath) {
    ...
    if (params.autoExposure) {
      ...
      applyScanFilmBaseCalibration(measuredParams, sourcePixels.data(), width, height);
      autoExposureEv = measureAutoExposureEv(sourcePixels.data(), width, height, measuredParams);
      ...
      applyScanFilmBaseCalibration(
        measuredParams,
        static_cast<const float *>(coreFrame.sourceStaging.mapped),
        width,
        height
      );
    }
  ...
  if (frameParamsEnabled) {
    ...
    frameFloats[105] = measuredParams.scanFilmBaseR;
    frameFloats[106] = measuredParams.scanFilmBaseG;
    frameFloats[107] = measuredParams.scanFilmBaseB;
    frameFloats[108] = params.scanBlackFlareR;
    frameFloats[109] = params.scanBlackFlareG;
    frameFloats[110] = params.scanBlackFlareB;
    frameFloats[111] = params.scanExposureEv;
    frameFloats[118] = params.scanDensityContrast;
    frameFloats[112] = params.scanDensityScaleR;
    frameFloats[113] = params.scanDensityScaleG;
    frameFloats[114] = params.scanDensityScaleB;
    frameFloats[115] = params.scanDensityOffsetR;
    frameFloats[116] = params.scanDensityOffsetG;
    frameFloats[117] = params.scanDensityOffsetB;
    ...
    frameInts[29] = static_cast<uint32_t>(params.printSource);
    frameInts[30] = static_cast<uint32_t>(params.scanInputEncoding);
    frameInts[31] = static_cast<uint32_t>(params.scanDensityBasis);
    frameInts[32] = static_cast<uint32_t>(params.scanInputColorSpace);
    frameInts[33] = static_cast<uint32_t>(params.scanWorkingColorSpace);
    frameInts[34] = static_cast<uint32_t>(params.inputPrimariesColorSpace);
    frameInts[35] = static_cast<uint32_t>(params.inputTransferColorSpace);
    frameInts[36] = static_cast<uint32_t>(params.outputPrimariesColorSpace);
    frameInts[37] = static_cast<uint32_t>(params.outputTransferColorSpace);
  ...
  if (printScanPassEnabled) {
    const VkDescriptorBufferInfo &printScanInputBufferInfo = (finalProcessNegative || scannedNegativePrintBypass)
      ? sourceBufferInfo
  ...
  pushConstants.inputColorSpace = static_cast<int32_t>(params.inputPrimariesColorSpace);
  pushConstants.inputPrimariesColorSpace = static_cast<int32_t>(params.inputPrimariesColorSpace);
  pushConstants.inputTransferColorSpace = static_cast<int32_t>(params.inputTransferColorSpace);
...
bool VulkanRenderer::Impl::renderTiledBootstrap(
  ...
  if (params.autoExposure) {
    ...
    if (fullPixelCount > static_cast<uint64_t>(std::numeric_limits<size_t>::max() / 4u / sizeof(float))) {
      lastError = "The Vulkan tiled scan analysis window is too large.";
      return false;
    }
    ...
    if (!copySourceToFloatStaging(source, window, fullWidth, fullHeight, fullSourcePixels.data())) {
      lastError = "The requested tiled scan analysis window does not fit inside the Vulkan source image view.";
      return false;
    }
    applyScanFilmBaseCalibration(tileParams, fullSourcePixels.data(), fullWidth, fullHeight);
    tileParams.filmExposureEv += measureAutoExposureEv(fullSourcePixels.data(), fullWidth, fullHeight, tileParams);
    tileParams.autoExposure = false;
  } else {
    applyScanFilmBaseCalibration(tileParams, nullptr, fullWidth, fullHeight);
  }
